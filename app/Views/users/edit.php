            ...
            <div class="card-body">
                ...
                <form action="<?= url_to('\App\Controllers\UsersController::update', $user->id) ?>" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="tel" class="form-control" id="phone" name="phone" value="<?= old('phone', esc($user->phone ?? '')) ?>">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="group" name="group">
                            ...
                            <?php 
                                $userGroup = !empty($user->getGroups()) ? $user->getGroups()[0] : '';
                            ?>
                            <?php foreach ($groups as $key => $group): ?>
                                <option value="<?= esc($key) ?>" <?= old('group', $userGroup) == $key ? 'selected' : '' ?>><?= esc($group['title']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-6 mt-4 ms-2 form-check form-switch px-4 py-6">
                        <input class="form-check-input switch-custom-size" type="checkbox" role="switch" id="active" name="active" value="1" <?= old('active', !$user->isBanned()) ? 'checked' : '' ?>>
                        <label class="form-check-label ms-2 pt-0 fw-semibold cursor-pointer" for="active">
                            Cuenta Activa
                        </label>
                    </div>
                    <div class="d-flex justify-content-center mt-4">
                        <a href="<?= url_to('\App\Controllers\UsersController::index') ?>" class="btn btn-danger px-4 me-2">Cancelar</a>
